...
# Alaska and Hawaii are intentionally omitted from this list: Alaska's bbox
# crosses the antimeridian (special-casing required) and both are rare
# targets for the Pi-sized offline stacks Geographica targets. Users needing
# them can add the entries; any bbox outside the contiguous 48 + DC is
# currently rejected at /api/start with a clear error.
#
# This primitive is imported from scripts/common/state_bboxes.py so that
# both the setup wizard and the pipeline container can use it (the pipeline
# mounts ./scripts but not ./setup).
