...
out_path = os.path.abspath("gx01-adapter-v2.kicad_pcb")
...
# U1 — RV-3028-C7 RTC, placed in the lower area below J2, clear of the
# LCD signal routing corridor. I²C1 runs across the middle of the board so
# the RTC's SDA/SCL traces are short.
u1 = load_footprint("Package_SON", "MicroCrystal_C7_SON-8_1.5x3.2mm_P0.9mm")
place(u1, "U1", "RV-3028-C7", 12.0, 27.0)

# C1 — CPH3225A supercap, custom footprint, adjacent to RTC
c1 = make_cph3225a_footprint()
place(c1, "C1", "1F 3.3V", 24.0, 27.0)

# U2 — BME280 placed in the bottom-right region so a case vent aligns with
# the sensor's air intake aperture. Well away from the Pi SoC hot spot.
u2 = load_footprint("Package_LGA", "Bosch_LGA-8_2.5x2.5mm_P0.65mm_ClockwisePinNumbering")
place(u2, "U2", "BME280", 54.0, 43.0)

# U3 — MCP9808 case air temp, right side mid-board
u3 = load_footprint("Package_SO", "MSOP-8_3x3mm_P0.65mm")
place(u3, "U3", "MCP9808", 36.0, 40.0)

# U4 — 24LC32 HAT ID EEPROM. Placed below J1's HAT ID pins 27/28 area
# (X = X_PIN1 + 13*PIN_PITCH ≈ 40.13).
u4 = load_footprint("Package_SO", "SOIC-8_3.9x4.9mm_P1.27mm")
place(u4, "U4", "24LC32", 45.0, 28.0)

# Q1 — AO3400 N-FET for LCD backlight PWM. Near J2's BLK pin region.
q1 = load_footprint("Package_TO_SOT_SMD", "SOT-23")
place(q1, "Q1", "AO3400A", 35.0, 20.0)

# C2 — bypass for U1 VDD
c2 = load_footprint("Capacitor_SMD", "C_0603_1608Metric")
place(c2, "C2", "100nF", 17.0, 25.0)

# C3 — bypass for U2 VDD
c3 = load_footprint("Capacitor_SMD", "C_0603_1608Metric")
place(c3, "C3", "100nF", 49.0, 40.0)

# C4 — bypass for U3 VDD
c4 = load_footprint("Capacitor_SMD", "C_0603_1608Metric")
place(c4, "C4", "100nF", 41.0, 41.0)

# C5 — bypass for U4 VCC
c5 = load_footprint("Capacitor_SMD", "C_0603_1608Metric")
place(c5, "C5", "100nF", 50.0, 32.0)

# R1, R2 — I²C1 pull-ups — place just below J1 near the I²C1 pins
# (J1 pin 3 at X=12.19, pin 5 at X=17.27 after 90° rotation)
r1 = load_footprint("Resistor_SMD", "R_0603_1608Metric")
place(r1, "R1", "4.7k", 13.0, 9.0, rotation_deg=0.0)

r2 = load_footprint("Resistor_SMD", "R_0603_1608Metric")
place(r2, "R2", "4.7k", 17.5, 9.0, rotation_deg=0.0)

# R3, R4 — I²C0 (HAT ID) pull-ups — place near J1 pins 27/28 (X≈40.13)
r3 = load_footprint("Resistor_SMD", "R_0603_1608Metric")
place(r3, "R3", "3.3k", 42.0, 9.0, rotation_deg=0.0)

r4 = load_footprint("Resistor_SMD", "R_0603_1608Metric")
place(r4, "R4", "3.3k", 46.0, 9.0, rotation_deg=0.0)

# R5 — Q1 gate pulldown, near Q1
r5 = load_footprint("Resistor_SMD", "R_0603_1608Metric")
place(r5, "R5", "10k", 39.0, 20.0, rotation_deg=0.0)

# R6, R7 — LCD V0 contrast divider, below J2's V0 (pin 3) and VEE (pin 18)
# After J2's 90° rotation, pin 3 is near X≈9.65, Y≈14, and pin 18 is near
# X≈30, Y≈14. Place dividers between them.
r6 = load_footprint("Resistor_SMD", "R_0603_1608Metric")
place(r6, "R6", "10k", 15.0, 20.0, rotation_deg=0.0)

r7 = load_footprint("Resistor_SMD", "R_0603_1608Metric")
place(r7, "R7", "10k", 25.0, 20.0, rotation_deg=0.0)
