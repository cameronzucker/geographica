...
**Sidebar overlay during active modes** (post-2026-04-25 redesign, surgical-scope 2026-04-25-phase3): ruler.js adds the `ruler-active` class to `document.body` in TWO independent code paths:

1. **renderPanel — for `state.status` === `drawing` or `inserting`.** These states involve sustained map-canvas interaction (vertex placement, insert-tap commit). The class makes `body.ruler-active #sidebar-overlay { pointer-events: none; }` (CSS rule) so map clicks fall through the invisible full-viewport sidebar-overlay (whose default behavior closes the sidebar on tap) and reach the MapLibre canvas. Without this, the first map tap during drawing would close the sidebar and hide the live vertex list / banner / Finish button.

2. **handleVertexMouseDown / handleTouchStart — surgical drag-only toggle.** When a vertex drag begins, the class is added directly so a drag mouseup released over the sidebar/overlay area still reaches the window-level mouseup listener (which commits the new vertex position). `cancelActiveDrag` removes the class on drag end, but ONLY when `state.status === 'editing'` — drawing/inserting states keep the class set via path #1.

**Why surgical instead of blanket-set across editing**: editing-no-drag should leave the sidebar dismissable via tap-outside-the-overlay (the user's quick non-destructive exit, north-star-aligned with Google Earth's "tap away to stop interacting"). Blanket-setting the class across all of editing (an earlier mid-Phase-3 attempt — commit `681485f`) disabled this affordance, and Cameron surfaced the regression during smoke 3. The drag-only toggle preserves both invariants: drag-mouseup reliability AND tap-outside-to-close.

Mirrors the existing `_bboxDrawingActive` pattern at [app.js:1206](frontend/app.js#L1206).
