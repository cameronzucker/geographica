...
**Sidebar overlay during active modes** (post-2026-04-25 redesign, scope-extended 2026-04-25-phase3): when `state.status` is `drawing`, `inserting`, OR `editing` (i.e., ANY non-`idle` state), ruler.js adds the `ruler-active` class to `document.body`. CSS rule `body.ruler-active #sidebar-overlay { pointer-events: none; }` lets map clicks AND drag-mouseups fall through the invisible full-viewport sidebar-overlay (whose default behavior closes the sidebar on tap) and reach the MapLibre canvas / window-level listeners. Without this:
- The first map tap during DRAWING would close the sidebar and hide the live vertex list / banner / Finish button.
- A vertex DRAG (Phase 3) released over the sidebar/overlay area in EDITING state would be swallowed before the window-level mouseup listener could commit the new vertex position.

The `editing` extension (added in Phase 3) trades tap-outside-to-close-sidebar UX during editing for drag-mouseup reliability. User must use the explicit hamburger / [×] / [Clear] / [+ New measurement] / Esc affordances to dismiss; the rationale is that ruler is a deliberate-attention task and accidental outside-taps shouldn't dismiss its UI mid-edit. Mirrors the existing `_bboxDrawingActive` pattern at [app.js:1206](frontend/app.js#L1206).
